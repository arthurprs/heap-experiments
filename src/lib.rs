    ...
    fn sift_down_range(&mut self, pos: usize, end: usize) {
        unsafe {
            ...
            while child < end {
                ...
                if D >= 4 && child + 3 < end {
                    if !(hole.get(child) > hole.get(child_base + 1)) {
                        child = child_base + 1;
                    }
                    if !(hole.get(child) > hole.get(child_base + 2)) {
                        child = child_base + 2;
                    }
                    if !(hole.get(child) > hole.get(child_base + 3)) {
                        child = child_base + 3;
                    }
                } else if D >= 3 && child + 2 < end {
                    if !(hole.get(child) > hole.get(child_base + 1)) {
                        child = child_base + 1;
                    }
                    if !(hole.get(child) > hole.get(child_base + 2)) {
                ...
                //     if i < end && !(hole.get(child) > hole.get(i)) {
                //         child = i;
                //     }
                // }
...
macro_rules! build {
    ($mod_name: ident, $class: ident, $N: expr) => {
        #[cfg(test)]
        mod $mod_name {
            use super::*;
            ...
            #[test]
            fn test_pop() {
                let mut heap = $class::new();
                for i in 0..100 {
                    heap.push(i);
                }
                for i in 0..100 {
                    assert_eq!(heap.pop().unwrap(), 99 - i);
                }
            }
            ...
            #[test]
            fn test_push() {
                let mut heap = $class::from(vec![2, 4, 9]);
                assert_eq!(heap.len(), 3);
            ...
            #[test]
            fn test_push_unique() {
                let mut heap = $class::<_>::from(vec![2, 4, 9]);
                assert_eq!(heap.len(), 3);
                assert!(*heap.peek().unwrap() == 9);
                heap.push(11);
                assert_eq!(heap.len(), 4);
            ...
            #[bench]
            fn bench_build_insert(b: &mut test::Bencher) {
                b.iter(|| {
                    let mut heap = $class::with_capacity(N2 as usize + 2);
                    let mut n = 0;
                    while n < N2 {
                        heap.push(n);
                        heap.push(N2 * 2 - n);
            ...
            #[bench]
            fn bench_from_vec(b: &mut test::Bencher) {
                b.iter(|| {
                    make_heap()
                })
            }
            ...
            #[bench]
            fn bench_insert_2000_prefilled(b: &mut test::Bencher) {
            ...
            #[bench]
            fn bench_insert_2000_empty(b: &mut test::Bencher) {
                b.iter(|| {
                    let mut heap = $class::with_capacity(2000);
                    for n in 0..1000 {
                        heap.push(n);
                        heap.push(N2 * 2 - n);
                    }
            ...
            #[bench]
            fn bench_pop_2000(b: &mut test::Bencher) {
                let mut heap = make_heap();
                b.iter(|| {
                    let mut heap = heap.clone();
                    for _ in 0..2000 {
            ...
            #[bench]
            fn bench_pop_all(b: &mut test::Bencher) {
                let mut heap = make_heap();
                b.iter(|| {
                    let mut heap = heap.clone();
            ...
            #[bench]
            fn pop_modify_push(b: &mut test::Bencher) {
                let mut heap = make_heap();
                b.iter(|| {
                    let mut heap = heap.clone();
                    for _ in 0 .. N {
            ...
            #[bench]
            fn peek_mut(b: &mut test::Bencher) {
                let mut heap = make_heap();
                b.iter(|| {
                    let mut heap = heap.clone();
                    for _ in 0 .. N {
                        heap.peek_mut().map(|mut elt| *elt = *elt + N2);
                    }
                });
                test::black_box(heap.pop());
            }
...
build!(std___heap, StdBinaryHeap, 256);
build!(d_ary_heap, BinaryHeap, 256);
